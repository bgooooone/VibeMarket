...
### 3.10 AI聊天历史表（ai_chat_history）

| 字段名 | 数据类型 | 约束 | 描述 |
| :--- | :--- | :--- | :--- |
| `id` | `BIGINT` | `PK, AI, NN` | 聊天记录ID |
| `user_id` | `BIGINT` | `FK, NN` | 用户ID |
| `message` | `TEXT` | `NN` | 用户发送的消息 |
| `ai_response` | `TEXT` | `NN` | AI助手的回复 |
| `session_id` | `VARCHAR(100)` | `NN` | 会话ID（用于标识同一对话） |
...
- 主键索引：`id`
- 外键索引：`user_id`
- 普通索引：`session_id`, `create_time`, `message_type`
...
CREATE TABLE `ai_chat_history` (
  ...
  `session_id` VARCHAR(100) NOT NULL COMMENT '会话ID（用于标识同一对话）',
