...
const routes = [
  ...
  },
  {
    path: '/ai/advice',
    name: 'AiAdvice',
    component: () => import('@/views/ai/Advice.vue'),
    meta: { requiresAuth: true }
  },
...
| API路径 | 方法 | 模块/文件 | 类型 | 功能描述 |
| :--- | :--- | :--- | :--- | :--- |
| `/api/v1/ai/chat` | `POST` | `AiAssistantController.java` | `Controller` | 智能聊天功能 |
| `/api/v1/ai/advice` | `GET` | `AiAssistantController.java` | `Controller` | 购物建议功能 |
...
public class AiChatHistory {
    ...
    private Integer messageType; // 1:普通聊天, 2:购物建议
    private Date createTime;
    private Date updateTime;
    // getter and setter
}
...
public class AiChatHistoryVO {
    ...
    public static AiChatHistoryVO convert(AiChatHistory history) {
        AiChatHistoryVO vo = new AiChatHistoryVO();
        vo.setId(history.getId());
        vo.setMessage(history.getMessage());
        vo.setAiResponse(history.getAiResponse());
        vo.setSessionId(history.getSessionId());
        vo.setMessageType(history.getMessageType());
...
public class EnhancedAiAssistantServiceImpl implements AiAssistantService {
    ...
    private String getProductBasedResponse(String userMessage) {
        try {
            ...
            // 3. 调用AI API
            return callQwenAPIWithContext(systemMessage, productPrompt);
...
1. **智能聊天流程**：
   - 用户登录后访问AI聊天页面
   - 用户输入消息并发送请求到`/api/v1/ai/chat`
   - 系统验证用户身份（JWT）
   - 调用AI助手服务与DashScope API交互
   - 返回AI响应给前端
   - 前端展示聊天内容

2. **购物建议流程**：
   - 用户登录后访问AI购物建议页面
   - 用户输入关键词并发送请求到`/api/v1/ai/advice`
   - 系统验证用户身份（JWT）
   - 调用AI助手服务获取购物建议
   - 返回购物建议给前端
   - 前端展示购物建议内容
...
import { chatWithAI, getShoppingAdvice } from '@/api/ai'
...
export const useAiStore = defineStore('ai', () => {
  ...
  // 获取购物建议
  async function fetchShoppingAdvice(keyword) {
    if (!keyword.trim()) return
    
    loading.value = true
    try {
      const response = await getShoppingAdvice(keyword.trim())
      return response.data
    } catch (error) {
      console.error('获取购物建议失败:', error)
      return null
    } finally {
      loading.value = false
    }
  ...
  async function loadChatHistory() {
    try {
      ...
      chatHistory.value = response.data.map(item => ({
        type: item.message ? 'user' : 'ai',
        content: item.message || item.aiResponse,
...
在前端路由配置中添加AI助手相关路由（已在2.1.4节中展示）。
...
CREATE TABLE `ai_chat_history` (
  ...
  `message` TEXT NOT NULL COMMENT '用户发送的消息',
  `ai_response` TEXT NOT NULL COMMENT 'AI助手的回复',
  `session_id` VARCHAR(100) NOT NULL COMMENT '会话ID',
  `message_type` TINYINT DEFAULT 1 COMMENT '消息类型（1:普通聊天, 2:购物建议）',
...
1. **API密钥保护**：
   - DashScope API密钥存储在配置文件中，使用环境变量或加密方式保护
   - 禁止在前端代码中直接暴露API密钥

2. **接口访问控制**：
   - 所有AI助手接口都需要用户登录（使用`@LoginRequired`注解）
   - 实现接口访问频率限制，防止恶意调用
...
2. **依赖配置**：
   - 在`pom.xml`中添加HTTP客户端依赖（如OkHttp或RestTemplate）

3. **环境变量配置**：
   - 生产环境中使用环境变量`DASHSCOPE_API_KEY`设置API密钥
...
2. **数据库优化**：
   - 对`ai_chat_history`表进行分区管理，按时间分区
   - 定期清理旧的聊天历史数据
...
本文档详细设计了校园购物商城的AI助手模块，包括后端接口设计、前端组件结构、数据库设计和安全性考虑。该模块集成了DashScope API，为用户提供智能聊天和购物建议功能，提升了用户体验。在实现过程中，需要注意API密钥的保护、用户隐私的数据安全以及系统的性能优化。
